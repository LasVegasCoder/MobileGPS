...
CSerialPort::CSerialPort(const std::string& device, SERIAL_SPEED speed, bool assertRTS) :
m_device(device),
m_speed(speed),
...
int CSerialPort::read(unsigned char* buffer, unsigned int length)
{
	assert(m_handle != INVALID_HANDLE_VALUE);
	assert(buffer != NULL);
...
int CSerialPort::readNonblock(unsigned char* buffer, unsigned int length)
{
	assert(m_handle != INVALID_HANDLE_VALUE);
	assert(buffer != NULL);
...
#if defined(__APPLE__)
int CSerialPort::setNonblock(bool nonblock)
{
